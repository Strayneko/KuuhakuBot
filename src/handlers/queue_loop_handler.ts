...
export default function queueLoopHandler(msg: Message, cmdArg: string) {
    ...
    if (cmdArg.length === 0) {
        (msg.channel as TextChannel).send(lang.EN.QUEUE.MISSING_REPEAT_ARG);
        return;
    }
    ...
    if (currentRepeatMode === repeatMode) {
        ...
        (msg.channel as TextChannel).send(`${lang.EN.QUEUE.SAME_REPEAT_ARG}${repeateModeStatus}`);
        return;
    }

    queue.setRepeatMode(repeatMode)
    if (cmdArg.toUpperCase() === 'ON') {
        (msg.channel as TextChannel).send(lang.EN.QUEUE.REPEAT_MODE_ON);
        return;
    }
    
    (msg.channel as TextChannel).send(lang.EN.QUEUE.REPEAT_MODE_OFF);
} 